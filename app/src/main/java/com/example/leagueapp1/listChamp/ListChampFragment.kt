package com.example.leagueapp1.listChamp
...
import com.example.leagueapp1.champListRecyclerView.ChampionListAdapterNoHeader
...
class ListChampFragment : Fragment() {
    ...
    private val viewModel: ListChampViewModel by viewModels()
    ...
    override fun onCreateView(
        ...
        val championListAdapter = ChampionListAdapterNoHeader() {
        ...
        viewModel.championList.observe(viewLifecycleOwner) { result ->
            ...
            when (result) {
                is Resource.Success -> {
                    ...
                    showChampList(result = result, adapter = championListAdapter)
                }
                is Resource.Loading -> {
    ...
    private fun showChampList(result: Resource<List<ChampionMastery>>, adapter: ChampionListAdapterNoHeader){
        setupAdapter(result.data!!, adapter)
        binding.progressBar.visibility = INVISIBLE
        binding.textViewError.visibility = INVISIBLE
    }

    private fun setupAdapter(data: List<ChampionMastery>, adapter: ChampionListAdapterNoHeader) {
